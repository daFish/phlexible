...
class PhpClassGenerator
{
    ...
    private function generateClass($className, array $data)
    {
        ...
        $classx = Object::make($className)
            ->addMethod(Method::make('__construct'))
            ->makeFinal()
            ->setPhpDoc(StructurePhpdoc::make()
                ->setDescription(Description::make("DO NOT EDIT THIS FILE - IT WAS CREATED BY PHLEXIBLE'S PROXY GENERATOR")
                    ->addLine(get_class($this))
                )
            );
        ...
        foreach ($items as $item) {
            ...
            $classx->addProperty(Property::make(lcfirst($valueName))
                ->makePrivate()
                ->setPhpdoc(PropertyPhpdoc::make()
                    ->setVariableTag($dataType)
                )
            );

            $classx->addMethod(Method::make($getterName)
                ->setBody("return \$this->$valueName;")
                ->setPhpdoc(MethodPhpdoc::make()
                    ->setReturnTag(new ReturnTag($dataType))
                )
            );

            $classx->addMethod(Method::make($setterName)
                ->addArgument(Argument::make($dataType, $valueName))
                ->setBody("\$this->$valueName = \$$valueName;\n\nreturn \$this;")
                ->setPhpdoc(MethodPhpdoc::make()
                    ->addParameterTag(ParameterTag::make($dataType, "\$$valueName"))
                    ->setReturnTag(new ReturnTag('$this'))
                )
            );
