...
class FieldMapper
{
     ...
     * @return array
     */
    public function extract(ElementVersion $elementVersion, $language)
    {
            ...
            if ($mappings) {
                foreach ($mappings as $key => $mapping) {
                    if ($mapper = $this->findFieldMapper($key)) {
                        $title = $mapper->map($content, $language, $mapping);
                        if ($title) {
                            $titles[$key] = $title;
                        }
                    }
        ...
        if (empty($titles['backend'])) {
            $titles['backend'] = '[' . $elementtype->getUniqueId() . ', ' . $language . ']';
        }
