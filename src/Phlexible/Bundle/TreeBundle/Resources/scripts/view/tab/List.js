Ext.provide('Phlexible.tree.view.tab.List');
...
Phlexible.tree.view.tab.List = Ext.extend(Ext.Panel, {
    ...
    initComponent: function () {
            ...
            },
            {
                ...
                selModel: new Ext.grid.RowSelectionModel({
                    ...
                    listeners: {
                        selectionchange: function (sm) {
                            ...
                            if (records.length && this.element.getNodeId()) {
                                var allowDuplicate = this.element.isGranted('CREATE'),
                                    allowPublish = true,
                                    allowDelete = true;
                                ...
                                if (this.mode == 'node') {
                                    ...
                                    allowPublish = this.element.isGranted('PUBLISH');
                                    allowDelete = this.element.isGranted('DELETE');
                                }
                ...
                listeners: {
                    rowdblclick: function (grid, index) {
                        ...
                        if (r.data.teaserId) {
                            this.fireEvent('listLoadTeaser', r.data.teaserId);
                        }
                        else {
                            this.fireEvent('listLoadNode', r.data.nodeId);
                        }
                    },
        ...
        Phlexible.tree.view.tab.List.superclass.initComponent.call(this);
    },
    ...
    doLoad: function (element) {
        ...
        cols.elements[5].update(this.dateRenderer(Date.parseDate(element.getTreeNode().attributes.customDate, 'Y-m-d')));

        var sortIcon;
        if (element.getTreeNode().attributes.sortMode == 'title') {
            if (element.getTreeNode().attributes.sortDir == 'desc') {
                sortIcon = 'p-element-sort_alpha_up-icon';
            } else {
                sortIcon = 'p-element-sort_alpha_down-icon';
            }
        }
        else if (element.getTreeNode().attributes.sortMode == 'createdate' || element.getTreeNode().attributes.sortMode == 'publishdate' || element.getTreeNode().attributes.sortMode == 'customdate') {
            if (element.getTreeNode().attributes.sortDir == 'desc') {
        ...
        this.updateSortButton(element.getTreeNode().attributes.sortMode, element.getTreeNode().attributes.sortDir);
    ...
    onPublishSort: function () {
        ...
        var params = {
            ...
            area_id: this.element.getAreaId() || null,
            mode: this.sortMode,
            dir: this.sortDir,
    ...
    duplicateNode: function () {
        ...
        var parent_tid = this.element.getNodeId();
        ...
        Ext.Ajax.request({
            ...
            success: function (response) {
                ...
                if (data.success) {
                    this.fireEvent('listReloadNode', this.element.getNodeId());
                        ...
                        success: function (response) {
                            ...
                            if (!result.data.length) {
                                ...
                                var w = new Phlexible.elements.DeleteInstancesWindow({
                                    data: result.data,
                                    parentId: this.element.getNodeId()
                                });
                                w.show();
                            }
                        },
...
Ext.reg('tree-tab-list', Phlexible.tree.view.tab.List);
