Ext.provide('Phlexible.tree.view.tab.Security');

Phlexible.tree.view.tab.Security = Ext.extend(Ext.form.FormPanel, {
    ...
    initComponent: function () {
        ...
        Phlexible.tree.view.tab.Security.superclass.initComponent.call(this);
    },
    ...
    onLoadElement: function (element) {
        ...
        if (this.element.getConfiguration().security && this.element.getConfiguration().security.roles) {
            var store = this.getComponent(1).getStore(),
                roles = [];
            Ext.each (this.element.getConfiguration().security.roles, function(role) {
                roles.push({role: role, name: role});
            }, this);
            store.loadData(roles);
        }

        this.getForm().setValues(this.element.getConfiguration().security || {});
    },
...
Ext.reg('tree-tab-security', Phlexible.tree.view.tab.Security);
