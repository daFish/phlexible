    ...
    initComponent: function () {
        ...
        this.element.on({
            createElement: function (element, data, node) {
                if (element.getLanguage() === data.master_language) {
            ...
            xxxpublish: function (element, result) {
                if (!element.getTeaserId()) {
                    if (element.getTreeNode()) {
                        ...
                        element.getTreeNode().attributes.navigation = data.navigation;
                        element.getTreeNode().attributes.restricted = data.restricted;
                        element.getTreeNode().setText(data.title);
                        var iconEl = element.getTreeNode().getUI().getIconEl();
                        if (data.status) {
        ...
        this.on('contextmenu', function (node, event) {
            ...
            if (node.attributes.permissions.indexOf('EDIT') !== -1) {
            ...
            if (Phlexible.User.isGranted('ROLE_ELEMENT_CREATE') &&
                node.attributes.permissions.indexOf('CREATE') !== -1) {
            ...
            if (Phlexible.User.isGranted('ROLE_ELEMENT_DELETE') &&
                node.attributes.permissions.indexOf('DELETE') !== -1) {
                if (!node.attributes.is_published ||
                    (Phlexible.User.isGranted('ROLE_ELEMENT_PUBLISH') &&
                    node.attributes.permissions.indexOf('PUBLISH') !== -1)) {
...
//	    });
        ...
        Phlexible.tree.view.Tree.superclass.initComponent.call(this);
...
//        }, this);
    ...
    initEvents: function () {
        ...
        if ((this.enableDD || this.enableDrop) && !this.dropZone) {
             ...
             */
            this.dropZone = new Phlexible.tree.tree.TreeDropZone(this, {
    ...
    xinitEvents: function () {
        ...
        //this.dropZone.setPadding(0, 0, this.getInnerHeight(), 0);
//        debugger;
        Phlexible.tree.tree.Tree.superclass.initEvents.call(this);
    },
    ...
    onDeleteElement: function (node) {
        ...
        var select = false;
        if (this.element.getTreeNode() && (node.id == this.element.getTreeNode().id || this.element.getTreeNode().isAncestor(node))) {
            select = true;
        }
